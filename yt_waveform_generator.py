...
Usage:
    python yt_waveform_generator.py --bits 12 --size 256 --output waveforms_12bit.h
    python yt_waveform_generator.py --bits 8 --size 1024 --waves sine,triangle,heart
    
Requirements:
    pip install jinja2
    
Project structure:
    yt_waveform_generator.py    # Main script
    templates/
...
def main():
    ...
    print(
        f"   Paramètres: {args.bits} bits, {args.size} échantillons, plage 0-{generator.max_value}"
    )
    ...
    for name, (description, func) in selected_waves.items():
        ...
        if min_val < 0 or max_val > generator.max_value:
            print(
                f"     ⚠️  Attention: plage {min_val}-{max_val} (attendu: 0-{generator.max_value})"
            )
        else:
